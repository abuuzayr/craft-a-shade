      ...
      <Container>
        <div className={styles.grid}>
          <div className={styles.mainContent}>
            ...
            <h1 className={`${styles.title} text-cas`}>{title}</h1>
            {categories.length > 0 && (
              <div className={styles.categories}>
                <ul>
                  {categories.map(category => (
                    <li key={category._id} className="text-cas">{category.title}</li>
                  ))}
                </ul>
              </div>
            )}
        ...
        <div className="my-10 text-center text-cas">
          share this article
          <div>
            <button className="outline-none px-6 py-4 font-bold border-primary border rounded-2xl m-2 text-sm hover:text-white hover:bg-cas" onClick={copy}>
              <div className="flex">
                {
                  copied ?
                    <>Article link copied!</> :
